...
export const DayIndicator = ({ days, selectedDay, onDayClick = () => { } }: { days: Date[]; selectedDay: Date; onDayClick: (day: Date) => void }) => {
    ...
    return (
        <div className='flex flex-row justify-between items-center my-5'>{days.map((day, index) => {
            ...
            return (
                <div onClick={() => onDayClick(day)} className={`relative text-center flex flex-col items-center justify-center h-12 w-12 sm:h-22 sm:w-22 rounded-lg bg-sidebar mx-1 cursor-pointer ${selectedDayStr === currentDayStr ? ' border-primary border-2' : ''}`} key={index}>
